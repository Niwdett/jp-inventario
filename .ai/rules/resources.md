...
- Flashes de sesión: los maneja `<x-toaster />` (único, en `layouts/app.blade.php`) como toast arriba-derecha. NO poner `@if (session('status'))` / `@if (session('error'))` con `<x-alert>` en las vistas — basta con `redirect()->with('status'|'error', ...)` en el controlador. Éxito se autodescarta (~4.5s); error se queda hasta cerrarlo. Desde JS: `window.dispatchEvent(new CustomEvent('toast', { detail: { variant, message, timeout } }))`. `<x-alert>` sigue para avisos persistentes en la página (p. ej. banner de stock bajo en `alertas/index`), no para flashes.
